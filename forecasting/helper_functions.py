...
def split_last_n_by_grain(df_input, n, time_colname, grain_colnames=None):
    """
    Group data frame by grain and split on last n rows for each group.

    :param df_input: input data frame
    :param n: number of observations in the test set
    :param time_colname: time column
    :param grain_colnames: (optional) list of grain column names
    :return train and test data frames
    """
    if grain_colnames is None:
        grain_colnames = []
    grain_colnames_original = grain_colnames.copy()
    if len(grain_colnames) == 0:
        grain_colnames = ['Grain']
        df_input[grain_colnames[0]] = 'dummy'
    # Sort by ascending time
    df_grouped = (df_input.sort_values(time_colname).groupby(grain_colnames, group_keys=False))
    df_head = df_grouped.apply(lambda dfg: dfg.iloc[:-n])
    df_tail = df_grouped.apply(lambda dfg: dfg.iloc[-n:])
    # drop group column name if it was not originally provided
    if len(grain_colnames_original) == 0:
        df_head.drop(grain_colnames, axis=1, inplace=True)
        df_tail.drop(grain_colnames, axis=1, inplace=True)
    return df_head, df_tail


def compute_grain_metrics(fcst_df, metric_name=None, grain_colnames=None):
    """
    ...
    if len(grain_colnames) == 0:
        grain_colnames = ['Grain']
        fcst_df[grain_colnames[0]] = 'dummy'
    metrics_list = []
    for grain, df in fcst_df.groupby(grain_colnames):
        try:
        ...
        if len(grain_colnames) < 2:
            one_grain_metrics_df['grain'] = grain_colnames[0]
        else:
